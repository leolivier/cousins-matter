...
description: "Creates Playwright-based UI tests for Django apps that don't have them yet. Use when the user asks to \"create UI tests\", \"add Playwright tests\", \"write UI tests for [app]\", \"écrire des tests UI\", or mentions testing user interfaces with Playwright in this project."
---
...
1. **`__init__.py` files in `{app}/tests/ui/` are empty (0 bytes)**. Do not add imports. Import `PlaywrightTestCase` directly from `core.tests.ui`.

2. **Always put `os.environ["DJANGO_ALLOW_ASYNC_UNSAFE"] = "true"` at the top of every file** (right after standard imports, before any Django import). This is mandatory because Playwright is synchronous and Django 6.x requires this variable.

3. **Pattern B MUST re-create `self.errors` and the `pageerror` listener** in its `setUp`, because it overrides `PlaywrightTestCase.setUp` where these are normally initialized.

4. **Validate factory field values** against model `choices` before using them. For example, `TroveFactory` uses `category="history"` (valid), `ClassifiedAdFactory` uses `category="home"` (valid). Pass invalid choices and the factory will raise a validation error.

5. **For form tests**, always check:
   - Visible fields (input, select, textarea)
   - Submit button
   - CSRF token
   - Cancel button/link if present

6. **For apps with complex JavaScript** (e.g., `chat` with WebSockets, gallery slideshow), start with simple display tests first. Complex interactions may require `page.evaluate()` calls like the ones in `galleries/tests/ui/tests_ui_slideshow.py`.

7. **Do not forget to import `reverse`** — every test file needs it.

8. **Avoid modifying `core/tests/ui/test_ui_base.py`** unless absolutely necessary. This class is shared by all apps.
