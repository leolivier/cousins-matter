...
class ChatConsumer(AsyncWebsocketConsumer):
  ...
  async def check_user_permission(self, msgid):
    ...
    try:
      ...
      await self.send(text_data=json.dumps({"action": "error", "error": _("An error occurred while checking permissions.")}))
      return
  ...
  async def receive_update_chat_message(self, data):
    ...
    try:
      ...
      await self.send(
        text_data=json.dumps({"action": "error", "error": f"{_('An error occurred while updating the message')}: {str(e)}"})
      )
  ...
  async def receive_delete_chat_message(self, data):
    ...
    try:
      ...
      await self.channel_layer.group_send(
        self.room_group_name, {"type": "update_chat_message", "msgid": msgid, "message": del_msg}
      )

    except Exception as e:
      logger.error(f"Error marking message {msgid} as deleted: {str(e)}")
      await self.send(
        text_data=json.dumps({"action": "error", "error": f'_("An error occurred while deleting the message"): {str(e)}'})
      )
