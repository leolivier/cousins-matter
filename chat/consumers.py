...
class ChatConsumer(AsyncWebsocketConsumer):
    ...
    async def create_message(self, member_id, room_slug, msg_content):
        ...
        message = await ChatMessage.objects.acreate(
            member=member, room=room, content=msg_content
        )
    ...
    async def receive_create_chat_message(self, data):
        ...
        await self.channel_layer.group_send(
            self.room_group_name,
            {
                ...
                "unformated_date_added": msg.date_added.strftime(
                    "%Y-%m-%d"
                ),  # for checking if the date has changed
    ...
    async def create_chat_message(self, event):
        ...
        """
        await self.send(
            text_data=json.dumps({"action": "create_chat_message", "args": event})
        )
    ...
    async def update_chat_message(self, event):
        ...
        # Send message to WebSocket
        await self.send(
            text_data=json.dumps({"action": "update_chat_message", "args": event})
        )
