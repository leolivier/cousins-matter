...
<form id="add-member-form" method="post" action="{reverse("chat:add_member_to_private_room", args=[self.room.slug])}">
  ...
  <button class="button" type="submit" disabled="true" id="add-member-button">
    ...
    response = self.client.post(
      reverse("chat:remove_member_from_private_room", args=[self.room.slug, self.created_members[1].id]), follow=True
    )
    # self.print_response(response)
    ...
    self.assertContainsMessage(response, "success", _("%s has been removed from the room") % self.created_members[1].full_name)
  ...
  def test_list_private_admins(self):
    ...
    response = self.client.post(
      reverse("chat:add_admin_to_private_room", args=[self.room.slug]), {"member-id": self.created_members[0].id}, follow=True
    )
...
<form id="add-member-form" method="post" action="{reverse("chat:add_member_to_private_room", args=[self.room.slug])}">
  ...
  def test_remove_member_from_private_room(self):
    ...
    response = self.client.get(
      reverse("chat:remove_member_from_private_room", args=[self.room.slug, first_member.id]), follow=True
    )
    self.assertContainsMessage(
      response,
      "error",
    ...
    # and retest
    response = self.client.get(
      reverse("chat:remove_member_from_private_room", args=[self.room.slug, second_member.id]), follow=True
    )
    # self.print_response(response)
  ...
  def test_leave_private_room(self):
    ...
    # self.print_response(response)
    self.assertContainsMessage(
      response, "error", _("You are the only member in this private room. Please add another one before removing yourself.")
    )
    # now, add a random member to self.room
    second_member = random.choice(self.created_members)
    self.room.followers.add(second_member)
...
<div class="mr-1 buttons has-addons is-rounded">
  ...
  <a class="button" href="{reverse("chat:private_room_admins", args=[self.room.slug])}">
    ...
    response = self.client.post(
      reverse("chat:add_member_to_private_room", args=[self.room.slug]), {"admin-id": self.member.id}, follow=True
    )
