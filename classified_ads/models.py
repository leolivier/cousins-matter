...
class Categories:
  ...
  def list_categories() -> list[tuple[str, str]]:
    if Categories.cat_list is None:
      Categories.cat_list = [(c, v['translation']) for c, v in Categories.CATEGORIES.items()]
  ...
  def list_subcategories(category) -> list[tuple[str, str]]:
    return [(k, v) for k, v in Categories.CATEGORIES[category]['subcategories'].items()]
...
class ClassifiedAd(models.Model):
    ...
    shipping_method = models.CharField(max_length=255, choices=SHIPPING_METHODS, default='pickup')
    location = models.CharField(max_length=255, blank=True, null=True)
    price = models.CharField(max_length=25)
    item_status = models.CharField(max_length=255, choices=ITEM_STATUS_TYPES, default='new')
    ad_status = models.CharField(max_length=255, choices=AD_STATUSES, default='for_sale')
...
class AdPhoto(models.Model):
  ...
  ad = models.ForeignKey(ClassifiedAd, related_name='photos', on_delete=models.CASCADE)
