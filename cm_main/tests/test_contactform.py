...
class TestContactForm(MemberTestCase):
    def test_contact_form(self):
        ...
        response = self.client.post(
            reverse("cm_main:contact"), {"message": test_msg}, follow=True
        )
        self.assertEqual(response.status_code, 200)
        ...
        self.assertEqual(
            Member.objects.filter(is_superuser=True).first(), self.superuser
        )
        ...
        msg = _(
            "%(sender_name)s sent you the following message from %(site_name)s:"
        ) % {
