...
"""
  ...
  def setUp(self):
    ...
    self.genealogy_url = self.build_url("genealogy:dashboard", _("Genealogy"), "family-tree")
...
class TestFeaturedPolls(TestFeaturedMixin):
  ...
  def test_polls_and_event_planners_enabled(self):
    ...
    self.assertContains(response, f"""<a  href="{lep_url}">{_("Event planners")}</a>""", html=True)
    response = self.client.get(reverse("polls:list_event_planners"))
    self.assertEqual(response.status_code, 200)
    poll_url = reverse("polls:list_polls")
    self.assertContains(response, f"""<a  href="{poll_url}">{_("Polls")}</a>""", html=True)
