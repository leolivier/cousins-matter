...
class TestMediaResourceMixin:
    def copy_test_resource(self, resource_file, target_file, return_content=False):
        ...
        if return_content:
            with open(
                resource_file, "rb"
            ) as initial_file:  # reopen as save above closes the file
...
class ProtectedMediaTestCase(TestMediaResourceMixin, MemberTestCase):
    def setUp(self):
        ...
        (self.test_file, self.uploaded_content) = self.copy_test_resource(
            logo_file, self.test_file, return_content=True
        )
