...
class TestFollowersMixin:
    def check_new_follower_email(
        self, follower, owner, followed_object, followed_url, expected_emails_count=1
    ):
        ...
        if followed_object == owner:
            message = _(
                ...
                "follower_url": get_test_absolute_url(
                    reverse("members:detail", args=[follower.pk])
                ),
    ...
    def check_new_content_email(
        ...
        self.assertEqual(
            len(follower_message.bcc), 2 if owner != follower else 1
        )  # only the follower and the owner
        if (
            followed_object == created_object
        ):  # e.g. when creating a chat room or a post
            subject = _('New %(followed_type)s "%(followed_object_name)s"') % {
                "followed_object_name": followed_object_name,
                "followed_type": followed_type,
            }
            message = _(
                "%(author_name)s created the following %(followed_type)s "
            ...
            message = _(
                "%(author_name)s added the following %(obj_type)s on %(followed_type)s "
                "<a href=\"%(followed_object_url)s\">'%(followed_object_name)s'</a>:"
            ) % {
  ...
  """
    ...
    def check_followers_emails(
        ...
        self.check_new_follower_email(
            follower, owner, followed_object, followed_url, expected_emails_count=2
        )
