...
# this view and tag allows including a view into another view. see template/core/base.html as an example
...
def get_view(parser, token):
  ...
  if len(tokens) < 2:
    raise TemplateSyntaxError(f"{token.contents.split()[0]} tag requires one or more arguments")
...
@register.inclusion_tag("core/common/inline_css.html")
