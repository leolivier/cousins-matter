...
def check_file_size(file, limit):
  if file.size > limit:
    ...
    raise ValidationError(
      _("Uploaded file %(filename)s is too big (%(sizemb)sMB), maximum is %(limitmb)sMB.")
...
def create_image(image_file, content_type="image/jpeg"):
  ...
  with Image.open(image_file) as img:
    ...
    return InMemoryUploadedFile(membuf, "ImageField", os.path.basename(image_file), content_type, size, None)
...
def allowed_date_formats():
  ...
  date_formats = [
    date_format for date_format in date_formats if not date_format.endswith("%f") and not date_format.endswith("%S")
  ]
...
def create_thumbnail(image: models.FileField, size: int) -> InMemoryUploadedFile:
  ...
  with Image.open(image.file) as imgf:
    img_width, img_height = imgf.size
    if img_height <= size or img_width <= size:
      return image
    imgf.thumbnail((size, size))
...
def confirm_delete_modal(request, title, msg, expected_value=None, hx_params=None):
  return render(
    request,
    "core/common/confirm-delete-modal.html",
