...
class PostFactory(DjangoModelFactory):
  ...
  def create_thread(self, create, extracted, **kwargs):
    ...
    for _ in range(random.randint(2, 8)):
      reply = MessageFactory(post=self, author=random.choice(members), content=fake.paragraph())

      # 50% chance of comments
      if random.choice([True, False]):
        for _ in range(random.randint(1, 3)):
          CommentFactory(message=reply, author=random.choice(members), content=fake.sentence())
