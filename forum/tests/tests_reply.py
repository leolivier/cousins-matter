...
class PostReplyTestCase(ForumTestCase):
    ...
    def test_edit_reply(self):
        ...
        msg = Message(
            content="a reply to be modified", post=self.post, author=self.member
        )
        msg.save()
    ...
    def test_delete_reply(self):
        ...
        msg = Message(
            content="a reply to be deleted", post=self.post, author=self.member
        )
        msg.save()
        ...
        self.assertJSONEqual(
            str(response.content, encoding="utf8"), {"reply_id": msg.id}
        )
