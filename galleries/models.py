...
def get_path(instance, filename, subdir=None):
  ...
  path = (
    os.path.join(settings.GALLERIES_DIR, instance.gallery.full_path(), subdir, filename)
    if subdir
...
class Photo(models.Model):
  ...
  uploaded_by = models.ForeignKey(
    "members.Member", verbose_name=_("Uploaded by"), on_delete=models.CASCADE, blank=True, null=True
  )
...
class Gallery(models.Model):
  ...
  cover = models.ForeignKey(
    "Photo", verbose_name=_("Cover Photo"), null=True, blank=True, on_delete=models.SET_NULL, related_name="cover_of"
  )
  parent = models.ForeignKey(
    "self", verbose_name=_("Parent gallery"), null=True, blank=True, on_delete=models.CASCADE, related_name="children"
  )
  slug = models.SlugField(max_length=70, blank=True, null=False)
  owner = models.ForeignKey("members.Member", verbose_name=_("Owner"), on_delete=models.CASCADE, blank=True, null=True)
      ...
      if Gallery.objects.filter(slug=slug, parent=self.parent).exists():
        if self.parent:
          raise ValidationError(
            _("Another sub gallery of %(parent)s with the same name already exists") % {"parent": self.parent.full_path()}
          )
        else:
