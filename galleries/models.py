...
def get_path(instance, filename, subdir=None):
    ...
    path = (
        os.path.join(
            settings.GALLERIES_DIR, instance.gallery.full_path(), subdir, filename
        )
        if subdir
        else os.path.join(
            settings.GALLERIES_DIR, instance.gallery.full_path(), filename
        )
    )
...
class Photo(models.Model):
    ...
    gallery = models.ForeignKey(
        "Gallery", verbose_name="Gallery", on_delete=models.CASCADE, blank=True
    )
...
class Gallery(models.Model):
            ...
            if Gallery.objects.filter(slug=slug, parent=self.parent).exists():
                if self.parent:
                    raise ValidationError(
                        _(
                            "Another sub gallery of %(parent)s with the same name already exists"
                        )
                        % {"parent": self.parent.full_path()}
                    )
                else:
                    raise ValidationError(
                        _("Another root gallery with the same name already exists")
                    )
            self.slug = slug
    ...
    def full_path(self):
        return (
            os.path.join(self.parent.full_path(), self.slug)
            if self.parent
            else self.slug
        )
    ...
    def cover_url(self):
        return (
            protected_media_url(self.cover.thumbnail.name)
            if self.cover
            else settings.DEFAULT_GALLERY_COVER_URL
        )
