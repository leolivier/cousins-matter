...
def create_photo(filename, filepath, zimport: ZipImport, gallery_id: str):
    ...
    date = (
        datetime.today()
        if date is None or date.startswith("0000")
        else datetime.strptime(date, "%Y:%m:%d %H:%M:%S").date()
    )
    # create image from in memory buffer
    image = InMemoryUploadedFile(
        membuffer, "ImageField", f"{filename_wo_ext}.jpg", "image/jpeg", size, None
    )
...
def post_create_photo(task: Task):
    logger.info(
        f"create photo {task.args} status: {task.success} result: {task.result} group: {task.group}"
    )
