...
def get_gallery_photos(gallery):
    ...
    """
    return (
        Photo.objects.filter(gallery=gallery)
        .order_by("id")
        .only("id", "name", "image", "thumbnail")
    )
    ...
    for idx, p in enumerate(page.object_list):
        ...
        if idx > 0:
            ...
            if idx == len(page.object_list) - 1 and page_num != num_pages:
                ...
                next_photo = (
                    Photo.objects.filter(gallery=p.gallery)
                    .order_by("id")
                    .filter(id__gt=p.id)
                    .first()
                )
                photos_dict[idx]["next_url"] = protected_media_url(
                    next_photo.image.name
                )
            ...
            if page_num > 1:  # not first page ==> take the previous photo
                ...
                photos_dict[idx]["previous_url"] = protected_media_url(
                    prev_photo.image.name
                )
            if (
                len(page.object_list) == 1 and page_num != num_pages
            ):  # only one photo per page and not last page
                next_photo = (
                    Photo.objects.filter(gallery=p.gallery)
                    .order_by("id")
                    .filter(id__gt=p.id)
...
def include_photos(gallery, page_num, page_size):
    ...
    ptor = Paginator(
        photos,
        page_size,
        compute_link=lambda page: reverse(
            "galleries:detail_page", args=[gallery.id, page]
        ),
    )
