...
class TestGalleryMoves(GalleryBaseTestCase):
  ...
  def test_photo_gallery_change_moves_file(self):
    ...
    if old_thumb_name:
      ...
      self.assertFalse(
        default_storage.exists(old_thumb_name) and old_thumb_name != new_thumb_name, "old thumb should be deleted if different"
      )

    self.assertTrue(
      "RootB" in new_image_name or "rootb" in new_image_name.lower(), "new image path should contain new gallery slug"
    )
