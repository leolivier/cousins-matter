...
class CreateGalleryTest(GalleryBaseTestCase):
    ...
    def test_create_sub_gallery(self):
        ...
        self.assertTrue(
            Gallery.objects.get(name=rgal_name, parent=None).children.first().name
            == sgal_name
        )
  ...
  <div class="control">
    <div class="select">
      <select name="cover" id="id_cover">
        ...
        response = self.client.post(
            url, {"name": gal_name, "description": "a test root gallery"}, follow=True
        )
    ...
    def test_create_sub_gallery(self):
        ...
        self.assertRedirects(
            response, reverse("galleries:detail", args=[sg.id]), 302, 200
        )
  ...
  <p>{sg.description}</p>
      ...
      {icon('back')}
        ...
        response = self.client.get(
            reverse("galleries:detail", args=[rg.id]), follow=True
        )
        # self.print_response(response)
  ...
  <div class="control">
    <div class="select">
      <select name="cover" id="id_cover">
        ...
        gal_name = get_gallery_name()
        response = self.client.post(
            url, {"name": gal_name, "description": rg.description}, follow=True
        )
        self.assertTrue(response.status_code, 200)
    ...
    def rec_build_tree(self, n, N=0, parent=None):
        ...
        gal = Gallery(
            name=name, parent=parent, description=f"a test gallery named {name}"
        )
        gal.save()
        url = reverse("galleries:detail", args=[gal.id])
        left_margin = (
            "".join(['<div class="ml-6">&nbsp;</div>' for i in range(N - 1)])
            + f'<div class="ml-6">{icon("menu-right")}</div>'
            if N
            else ""
        )
...
class DeleteGalleryViewTest(GalleryBaseTestCase):
    ...
    def test_delete_gallery(self):
        ...
        self.assertContainsMessage(
            response, "success", _("Gallery deleted successfully")
        )
