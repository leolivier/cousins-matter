...
class PhotoTestsBase(GalleryBaseTestCase):
    def setUp(self):
        ...
        self.root_gallery = Gallery(
            name=get_gallery_name(), description="a test root gallery"
        )
...
class CreatePhotoTests(PhotoTestsBase):
    def test_create_photo_no_gallery(self):
        ...
        with self.assertRaises(ValidationError):
            p = Photo(
                name=get_photo_name(), date=date.today(), image=self.image, gallery=None
            )
            p.save()
    ...
    def test_create_photo_no_date(self):
        ...
        with self.assertRaises(ValidationError):
            p = Photo(
                name=get_photo_name(), image=self.image, gallery=self.root_gallery
            )
            p.save()
...
class CreatePhotoViewTests(PhotoTestsBase):
    def test_create_photo_view(self):
        ...
        ap_url = reverse(
            "galleries:add_photo", kwargs={"gallery": self.root_gallery.id}
        )
    ...
    def get_photo_dicts(self, photos, nb_photos, page_num, last):
        ...
        if page_num > 1:
            del photo_dicts[
                0
            ]  # remove the first photo of the page which is indeed the last of the previous page
        if last < nb_photos:  # not last page
            del photo_dicts[
                -1
            ]  # remove the last photo of the page which is indeed the first of the next page
        return photo_dicts

    def check_display_several_photos(
        self, page_size, nb_photos, page_num=1, gallery=None
    ):
        ...
        if page_num > 1:
            url = (
                reverse(
                    "galleries:detail_page", kwargs={"pk": gallery.id, "page": page_num}
                )
                + f"?page_size={page_size}"
            )
        else:
            url = (
                reverse("galleries:detail", kwargs={"pk": gallery.id})
                + f"?page_size={page_size}"
            )
...
""",
        ...
        if nb_photos > (page_size * page_num):  # test next page
            self.check_display_several_photos(
                page_size, nb_photos, page_num + 1, gallery
            )
...
class PhotoEditViewTest(PhotoTestsBase):
    ...
    def test_edit_photo_no_owner(self):
        ...
        self.assertRedirects(
            response, reverse("galleries:photo", kwargs={"pk": self.photo.id}), 302, 200
        )
