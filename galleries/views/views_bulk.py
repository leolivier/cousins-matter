...
def _get_or_create_gallery(path: str, zimport: ZipImport):
    ...
    if ".." in pathlib.PurePath(path).parts:
        raise SuspiciousFileOperation(
            _(
                "Detected path traversal attempt, '..' is not allowed in paths inside the zip file"
            )
        )
    ...
    try:
        ...
        gallery = Gallery.objects.create(
            name=name, parent=parent, description=description, owner_id=zimport.owner_id
        )
...
def handle_zip(zip_file, task_group, owner_id):
    ...
    for dir, subdirs, files in os.walk(tmpdir):
        images = [
            file for file in files if mimetypes.guess_type(file)[0].startswith("image/")
        ]
        if len(images) == 0:  # create galleries only if there are photos inside
            continue
        gallery_path = os.path.relpath(
            dir, tmpdir
        )  # get relative path from temp to see the galleries path
...
def upload_progress(request, id):
    ...
    if value == max:  # reached the end
        ...
        context["success"] = _(
            "Zip file uploaded: %(lg)d galleries and %(nbp)d photos created"
