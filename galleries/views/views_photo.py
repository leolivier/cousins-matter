    ...
    def get(self, request, **kwargs):
        ...
        try:
            ...
            ptor = Paginator(
                photos,
                per_page=1,
                compute_link=lambda photo_num: reverse(
                    "galleries:photo_list", args=[gallery_id, photo_num]
                ),
            )
...
def get_photo_url(request, gallery, photo_idx=1):
    ...
    return JsonResponse(
        {"pk": photo.id, "image_url": protected_media_url(photo.image.name)}
    )
    ...
    def post(self, request, gallery, *args, **kwargs):
        ...
        if form.is_valid():
            try:  # issue 120: if any exception during the thumbnail creation process, remove the photo from the database
                ...
                logger.error(e)
                messages.error(
                    request, _("Error when creating this photo: %s.") % str(e)
                )
...
def delete_photo(request, pk):
    ...
    if not request.user.is_superuser:
        if (
            (photo.uploaded_by or photo.gallery.owner)
            and request.user != photo.uploaded_by
            and request.user != photo.gallery.owner
        ):
