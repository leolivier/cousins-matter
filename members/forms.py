...
class RegistrationRequestForm(Form):
    ...
    email = forms.EmailField(
        label=_("Email where you will receive the link"), max_length=254
    )
    message = forms.CharField(
        label=_("Message to the administrator"), widget=forms.Textarea, max_length=2000
    )
    captcha = CaptchaField(
        label=_("Captcha (click on the image to refresh if you can't read it)")
    )
...
class CSVImportMembersForm(forms.Form):
    ...
    activate_users = forms.BooleanField(
        label=_("Automatically activate imported users"), required=False
    )
