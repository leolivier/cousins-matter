...
class Address(models.Model):
    ...
    complementary_info = models.CharField(
        _("Complementary info"), max_length=120, default="", blank=True
    )
...
class Member(AbstractUser):
  ...
  avatar = models.ImageField(upload_to=settings.AVATARS_DIR, blank=True, null=True)
    ...
    privacy_consent = models.BooleanField(
        _("Privacy consent"), default=False, blank=False, null=False
    )
  ...
  objects = MemberManager()
    ...
    def clean(self):
        ...
        if self.is_active and self.member_manager is not None:
            ...
            logger.debug(
                f"Cleaning member {self.full_name}: changing member manager to admin"
            )
    ...
    def save(self, *args, **kwargs):
        ...
        if self.avatar:
            ...
            if saved_path != self.avatar_mini_name:
                logger.error(
                    f"ERROR: saved_path != self.avatar_mini_name: {saved_path} != {self.avatar_mini_name}"
                )
            logger.debug(
                f"Resized and saved avatar for {self.full_name} in {saved_path}, size: {settings.AVATARS_MINI_SIZE}"
            )
