...
from .models import ALL_FIELD_NAMES, MEMBER_FIELD_NAMES, ADDRESS_FIELD_NAMES, Member, Family, Address
...
def manage_avatar(row_data: MemberImportData):
  ...
  if not default_storage.exists(avatar):
    ...
    try:
      ...
      row_data.warnings.append(_("Error saving avatar (%(warning)s): %(avatar)s for username %(username)s. Ignored...") %
                               {'warning': e, 'avatar': avatar, 'username': username})


def manage_family(row_data: MemberImportData):
  family_name = row_data.row[t('family')]
  if not row_data.current_member.family or row_data.current_member.family.name != family_name:
    row_data.current_member.family, _ = Family.objects.get_or_create(name=family_name)
    row_data.set_updated()


def get_valid_manager(default_manager: Member, row_data: MemberImportData, manager_username):
  ...
  if not new_member_manager:  # should never happen!
    row_data.warnings.append(_("Manager %(manager)s not found for member %(member)s.") % {
      'manager': manager_username,
      'member': row_data.current_member.full_name} + ' ' + warning)
  elif not new_member_manager.is_active:
    row_data.errors.append(_("%(manager)s is inactive and cannot be used as manager for %(member)s. "
...
def handle_no_manager_case(context: ImportContext, row_data: MemberImportData):
  ...
  if context.activate_users:
    ...
    row_data.warnings.append(_("No manager provided for member %(member)s although inactive. "
    ...
    row_data.errors.append(_("Inactive member %(member)s has no manager. Please provide one! "
...
def handle_managed_by(context: ImportContext, row_data: MemberImportData):
  ...
  new_member_manager = get_valid_manager(context.default_manager, row_data, manager_username)
  ...
  if row_data.current_member.is_active:
    row_data.warnings.append(_("Member %(member)s was active. Adding %(manager)s as manager inactivated him/her.") %
