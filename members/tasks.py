...
def manage_avatar(row_data: MemberImportData):
  ...
  if not default_storage.exists(avatar):
    row_data.warnings.append(
      _("Avatar not found: %(avatar)s for username %(username)s. Ignored...") % {"avatar": avatar, "username": username}
    )
  else:
    try:
      ...
      row_data.warnings.append(
        _("Error saving avatar (%(warning)s): %(avatar)s for username %(username)s. Ignored...")
...
def handle_no_manager_case(context: ImportContext, row_data: MemberImportData):
  ...
  if context.activate_users:
    ...
    row_data.warnings.append(
      ...
      % {"member": row_data.current_member.full_name, "manager": row_data.current_member.member_manager.full_name}
    )
  else:
    row_data.errors.append(
...
def handle_managed_by(context: ImportContext, row_data: MemberImportData):
  ...
  if row_data.current_member.is_active:
    row_data.warnings.append(
      _("Member %(member)s was active. Adding %(manager)s as manager inactivated him/her.")
