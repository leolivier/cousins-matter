...
class TestAddress(MemberTestCase):
	def test_create_and_modify_address(self):
		...
		self.member.save()
		self.assertEqual(Address.objects.get(number_and_street = "1 Avenue des Champs-Elysées"), self.member.address)
		# test modify
...
class TestAddressView(MemberTestCase):
	...
	def test_create_address_view(self):
		...
		self.assertTemplateUsed(response,'members/address_form.html')
