...
class TestMemberFollowersMixin(TestFollowersMixin):
    ...
    def do_test_toggle_unfollow_member(self, followed):
        ...
        )  # login as follower (ie self.member)
        url = reverse("members:toggle_follow", args=[followed.id])
        response = self.client.post(url, follow=True)
        self.assertEqual(response.status_code, 200)
...
@django_q_sync_class
class TestMemberFollower(
    TestMemberFollowersMixin, ChatMessageSenderMixin, MemberTestCase
):
    def test_follow_member(self):
        """Tests that the follow member view works correctly."""
        follower = self.member
        followed = self.do_test_toggle_follow_member()
        followed_url = get_test_absolute_url(
            reverse("members:detail", args=[followed.id])
        )
        ...
        response = self.client.post(
            url, {"title": post_title, "content": post_content}, follow=True
        )
        self.assertEqual(response.status_code, 200)
...
@tag("needs-redis")
@django_q_sync_class
class TestChatWithMemberFollower(
    TestMemberFollowersMixin, ChatMessageSenderMixin, MemberTestCase
):
    # def tearDown(self):
    ...
    async def test_follow_room(self):
        ...
        response = await self.async_client.get(
            reverse("chat:new_room"), {"name": room_name}
        )
