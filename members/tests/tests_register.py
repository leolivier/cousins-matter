...
class MemberInviteTests(MemberTestCase):
  def do_test_invite(self, should_fail=False):
    ...
    self.assertInHTML(
      _('Registration to the "%(site_name)s" site') % {"site_name": settings.SITE_NAME},
      content,
    )
...
class RequestRegistrationLinkTests(TestLoginRequiredMixin, MemberTestCase):
  ...
  def test_request_registration(self):
    ...
    self.assertEqual(
      mail.outbox[0].subject,
      _("Registration request for %(site_name)s") % {"site_name": settings.SITE_NAME},
    )
    for content, type in mail.outbox[0].alternatives:
      if type == "text/html":
        break
    # print("content=", content)
    self.assertInHTML(
      _('Registration request for the "%(site_name)s" site') % {"site_name": settings.SITE_NAME},
      content,
    )
    self.assertInHTML(
