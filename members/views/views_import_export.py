        ...
        if fieldname not in fieldnames:
            raise ValidationError(
                ...
                % {
                    "fieldname": fieldname,
                    "all_names": ", ".join(
                        [str(s) for s in MANDATORY_MEMBER_FIELD_NAMES.values()]
                    ),
                }
            )
...
class CSVImportView(LoginRequiredMixin, generic.FormView):
    ...
    def get_context_data(self, *args, **kwargs):
        optional_fields = {str(s) for s in ALL_FIELD_NAMES.values()} - {
            str(s) for s in MANDATORY_MEMBER_FIELD_NAMES.values()
        }
        ...
        if form.is_valid():
            try:
                ...
                import_data = import_csv(
                    csv_file, task_group, request.user.id, activate_users
                )

                hx_get_url = reverse("members:import_progress", args=(task_group,))
                logger.debug(
                    f"rendering first progress-bar url: {hx_get_url} task group: {task_group}"
                )
                return render(
                    request,
...
def import_progress(request, id):
    ...
    if value == max:  # reached the end
        ...
        context["success"] = _(
            "CSV file uploaded: %(rows_num)i lines read, %(created_num)i members created "
    ...
    logger.debug(
        f"upload progress bar value: {value}, max: {max}, "
...
def select_family(request):
    ...
    families = (
        Family.objects.filter(name__icontains=query)
...
def select_city(request):
    ...
    cities = (
        Address.objects.filter(city__icontains=query)
...
def export_members_to_csv(request):
    ...
    members = (
        members.select_related("address")
        .select_related("family")
        .select_related("member_manager")
        .order_by("username")
    )
    ...
    for member in members:
        ...
        for field in MEMBER_FIELD_NAMES.keys():
            if field == "family":
                ...
                row.append(
                    member.member_manager.username if member.member_manager else ""
                )
            else:
