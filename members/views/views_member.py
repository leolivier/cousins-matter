...
def validate_username(request):
    ...
    response = {
        "is_taken": username != request.user.username
        and Member.objects.filter(username__iexact=username).exists()
    }
...
def member_manager_name(member):
    return (
        Member.objects.get(id=member.member_manager.id).full_name
        if member and member.member_manager
        else None
    )
...
class MembersView(LoginRequiredMixin, generic.ListView):
    ...
    def get(self, request, page_num=1):
        ...
        filters = {
            f"{name}_unaccent__icontains": remove_accents(
                request.GET[f"{name}_filter"]
            ).strip()
        ...
        order = (
            "-" if request.GET.get("member_order") == "option2" else ""
...
class MemberDetailView(LoginRequiredMixin, generic.DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        return super().get_context_data(**kwargs) | {
            ...
            "member_manager_name": member.member_manager.username
            if member.member_manager
            else None,
            "hobbies_list": [s.strip() for s in member.hobbies.split(",")]
            if member.hobbies
            else [],
        }
...
class EditMemberView(LoginRequiredMixin, generic.UpdateView):
    ...
    def get(self, request, pk):
        ...
        if not _can_edit_member(request, member):
            messages.error(
                request, _("You do not have permission to edit this member.")
            )
            return redirect("members:detail", member.id)
        ...
        return render(
            ...
            {
                "form": MemberUpdateForm(
                    instance=member, is_profile=self.is_profile_view
                ),
                "addr_form": AddressUpdateForm(),
                "family_form": FamilyUpdateForm(),
                "pk": pk,
    ...
    def post(self, request, pk):
        ...
        if form.is_valid():
            if (
                ...
                and "email" in form.changed_data
                and form.cleaned_data["email"]
            ):
                ...
                messages.info(
                    request,
                    _(
                        "A verification email has been sent to validate your new email address."
                    ),
                )
            else:
                try:
