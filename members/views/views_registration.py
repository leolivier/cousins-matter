...
class RegistrationCheckingView(generic.CreateView):
    ...
    def check_before_register(self, request, encoded_email, token):
        ...
        if not decoded_email:
            messages.error(
                request, _("Invalid link. Please contact the administrator.")
            )
            return False
        ...
        if member.exists():
            ...
            if member.is_active:
                messages.error(
                    request,
                    _(
                        "A member with the same email address is already active. Please sign in instead"
                    ),
                )
                return False
            ...
            messages.error(
                request,
                _(
                    "You are already registered but not active. Please contact %(admin)s to activate your account"
                )
...
class MemberInvitationView(LoginRequiredMixin, generic.View):
    ...
    def check_before_invitation(self, request):
        if (
            ...
            and not settings.ALLOW_MEMBERS_TO_INVITE_MEMBERS
        ):
