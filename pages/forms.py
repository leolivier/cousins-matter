...
class PageForm(FlatpageForm):
    ...
    def clean_url(self):
        ...
        ).filter(Q(url_start=F("url")) | Q(url__startswith=new_url))
        if self.instance.id:  # update, exclude the instance itself from the check
            new_url_starts_w_existing_urls = new_url_starts_w_existing_urls.exclude(
                pk=self.instance.id
            )
        ...
        if new_url_starts_w_existing_urls.exists():
            ...
            raise ValidationError(
                _(
                    "A flatpage cannot be a subpage of another flatpage, check your URLs"
                ),
                code="same_start_url",
            )
        return new_url
