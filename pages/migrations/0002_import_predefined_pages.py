...
def createPredefinedCustomFlatPages(apps, deserialized_pages):
  ...
  for deserialized_page in deserialized_pages:
    ...
    if db_base.exists():
      ...
      createCustomFlatPage(apps, base, predefined=custom.predefined, updated=custom.updated)
...
def migrateOrCreateCustomeFlatPages(apps, schema_editor):
  ...
  with open('pages/fixtures/predefined_flatpages.json') as fixture_file:
    ...
    for item in deserialized:
      ...
      if deserialized_obj._meta.app_label == 'flatpages' and deserialized_obj._meta.model_name == 'flatpage':
        # this is a base page, just remember it for when we'll read the corresponding custom page
        base_pages.append(deserialized_obj)
      elif deserialized_obj._meta.app_label == 'pages' and deserialized_obj._meta.model_name == 'flatpage':
        ...
        if bases:
          ...
          raise ValueError(f"Base flat page not found for page {deserialized_obj.pk}")
      else:
        raise ValueError(f"Unknown object type {type(deserialized_obj)} when deserializing predefined_flatpages.json")
...
class Migration(migrations.Migration):
  ...
  operations = [
    migrations.RunPython(migrateOrCreateCustomeFlatPages, reverse_code=reverseMigrateOrCreateCustomeFlatPages),
  ]
