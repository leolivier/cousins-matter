...
def reverseMigrateAdminMessages(apps, schema_editor):
    CustomFlatPage = apps.get_model("pages", "FlatPage")
    for page in CustomFlatPage.objects.filter(
        url__startswith=settings.ADMIN_MESSAGE_PAGE_URL_PREFIX
    ):
        page.url = page.url.replace(
            settings.ADMIN_MESSAGE_PAGE_URL_PREFIX, "/pages/admin-message"
        )
        page.save(update_fields=["url"])
    for page in CustomFlatPage.objects.filter(
        url__startswith=settings.MENU_PAGE_URL_PREFIX
    ):
...
class Migration(migrations.Migration):
    ...
    operations = [
        migrations.RunPython(
            migrateAdminMessages, reverse_code=reverseMigrateAdminMessages
        ),
    ]
