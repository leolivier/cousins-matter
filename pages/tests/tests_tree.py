...
class TestDisplayTreePage(TestPageMixin, BasePageTestCase, MemberTestCase):
    def create_pages(self):
        ...
        for i in range(5):
            ...
            levels = [kind] + [
                f"level-{i}" for i in random.sample(range(1, 4), nbl - 1)
            ]
            jlevels = "/".join(levels)
            data = {
                "url": f"/{jlevels}/{slug}/",
    ...
    def check_pages(self, response, pages, edit=False):
        ...
        for page in pages:
            if page.url.startswith(
                settings.MENU_PAGE_URL_PREFIX
            ) or page.url.startswith(settings.PRIVATE_PAGE_URL_PREFIX):
                url = (
                    reverse("pages-edit:update", kwargs={"pk": page.id})
                    if edit
                    else f"/{settings.PAGES_URL_PREFIX}{page.url}"
                )
  ...
  <span class="tag is-success is-light">
    <a href="{url}">
      {page.title}
                ...
                levels = list(
                    filter(lambda x: x is not None and x != "", page.url.split("/"))
                )
                nbl = len(levels)
...
<span class="tree-level">
  ...
  <span class="tag is-primary is-light">{level}</span>
    ...
    def test_display_tree_of_pages(self):
        ...
        )  # only superuser can create pages
        ...
        )  # relog as simple user to check tree
        response = self.client.get(reverse("pages-edit:tree"), follow=True)
