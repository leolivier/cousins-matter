...
def get_answerform_class_for_question_type(question_type):
    ...
    try:
        ...
        raise ValueError(
            f"No AnswerForm subclass found for question type {question_type}"
        )
...
class ChoiceAnswerForm(AnswerFormMixin, forms.ModelForm):
    answer = forms.ChoiceField(
        required=True, label=_("choice"), help_text=_("Select one choice")
    )
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["answer"].choices = [
            (choice, choice) for choice in self.question.possible_choices
        ]
