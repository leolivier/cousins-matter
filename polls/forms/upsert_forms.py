...
class EventPlannerUpsertForm(forms.ModelForm):
    multichoices_planner = forms.BooleanField(
        label=_("Allow choosing multiple dates in the answer"), required=False
    )
    possible_dates = forms.CharField(
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["possible_dates"].help_text = _(
            ...
            [
                translate_date_format(date_format)
        ...
        if "instance" in kwargs:
            date_question = Question.objects.filter(
                poll=kwargs["instance"], question_type__in=Question.EVENT_TYPES
            ).first()
            self.fields["possible_dates"].initial = "\n".join(
                date_question.possible_choices
            )
            self.fields["multichoices_planner"].initial = (
    ...
    def clean_possible_dates(self):
        ...
        if len(possible_dates) < 2:
            raise forms.ValidationError(
                _("You must provide at least two possible dates!")
            )
        for date in possible_dates:
            ...
            if datetime.datetime.now() > real_date:
                raise forms.ValidationError(
                    date + ": " + _("All dates must be in the future!")
                )
...
class QuestionUpsertForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if (
            "instance" in kwargs
            and kwargs["instance"].question_type in Question.CHOICE_TYPES
        ):
            self.fields["possible_choices"].initial = "\n".join(
                kwargs["instance"].possible_choices
            )
    ...
    def clean_possible_choices(self):
        ...
        possible_choices = [
            choice.strip() for choice in data.split("\n") if choice.strip()
        ]
        if len(possible_choices) < 2:
            raise forms.ValidationError(
                _("You must provide at least two possible choices!")
            )
