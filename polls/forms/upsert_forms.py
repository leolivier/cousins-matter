...
class EventPlannerUpsertForm(forms.ModelForm):
  ...
  class Meta:
    model = EventPlanner
    fields = ['title', 'description', 'location', 'possible_dates', 'multichoices_planner',
              'chosen_date', 'pub_date', 'close_date', 'open_to', 'closed_list']
    labels = {'title': _('Title'), 'pub_date': _('To be published on'), 'close_date': _('To be closed on'),
              'location': _('Location'), 'chosen_date': _('Chosen date'),
              'open_to_all': _('Open to all'), 'open_to_active': _('Open to active members'),
              'closed_list': _('Closed list')}

  def __init__(self, *args, **kwargs):
    super().__init__(*args, **kwargs)
    self.fields['possible_dates'].help_text = \
      _("Provide the possible dates and times, one per line. Allowed formats are: ") + \
...
class QuestionUpsertForm(forms.ModelForm):
  ...
  def __init__(self, *args, **kwargs):
    ...
    if 'instance' in kwargs and \
        kwargs['instance'].question_type in Question.CHOICE_TYPES:
