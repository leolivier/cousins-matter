...
class Poll(models.Model):
    ...
    open_to = models.CharField(
        max_length=3, choices=OPEN_TO_TYPES, default=OPEN_TO_ACTIVE
    )
...
class Question(models.Model):
    ...
    question_type = models.CharField(
        _("Question Type"), max_length=2, choices=QUESTION_TYPES, default=YESNO_QUESTION
    )
...
class Answer(models.Model):
    ...
    question = models.ForeignKey(
        Question, related_name="answers_%(class)s", on_delete=models.CASCADE
    )
    question_type = None
    answer = None
    subclasses = None
  ...
  def filter_answers(**kwargs):
    ...
    results = []  # List of matching instances
        ...
        for subclass in Answer.subclasses:
            ...
            queryset = (
                subclass.objects.filter(**kwargs) if kwargs else subclass.objects.all()
            )
            for instance in queryset:
                ...
                results.append(
                    instance
                )  # add them all at the start, we'll remove unwanted afterwards
        ...
        for numeric_id, instances_with_same_id in grouped_instances_by_id.items():
            ...
            for parent_candidate in instances_with_same_id:
                ...
                for child_candidate in instances_with_same_id:
                    ...
                    if parent_candidate is not child_candidate and issubclass(
                        child_candidate.__class__, parent_candidate.__class__
                    ):
                        results.remove(
                            parent_candidate
                        )  # remove the parent candidate from the results
                        break
    ...
    for numeric_id, instances_with_same_id in grouped_instances_by_id.items():
      ...
      for parent_candidate in instances_with_same_id:
        ...
        for child_candidate in instances_with_same_id:
          # If they are not the same instance and the child candidate is a subclass of the parent candidate
          if parent_candidate is not child_candidate and issubclass(child_candidate.__class__, parent_candidate.__class__):
  ...
  def all_answers():
    ...
    def get_answer_class_for_question_type(question_type):
        ...
        try:
            ...
            raise ValueError(
                f"No Answer subclass found for question type {question_type}"
            )
...
class QuestionResult:
  ...
  def __init__(self, question):
    ...
    def build_result(self, user=None):
        answer_class = Answer.get_answer_class_for_question_type(
...
class YesNoAnswer(Answer):
  ...
  def __str__(self):
    ...
    def compute_result(answers, result):
        ...
        return [
            f"{(answers.filter(answer=True).count() / result.total_answers) * 100 if result.total_answers > 0 else 0}%"
        ]
  ...
  def __str__(self):
    ...
    def compute_result(answers, result):
        ...
        return (
            ...
            else list(answers.values_list("answer", flat=True))
        )
    ...
    def compute_result(answers, result):
        ...
        for choice in result.question.possible_choices:
            ...
            choice_results[choice] = (
                (choice_answers / result.total_answers) * 100
                if result.total_answers > 0
                else 0
            )
        return (
            [f"{key}: {value}%" for key, value in choice_results.items()]
            if choice_results
            else ["-"]
        )
...
class EventPlanner(Poll):
    ...
    def get_questions(self):
        return Question.objects.filter(poll=self).exclude(
            question_type__in=Question.EVENT_TYPES
        )
