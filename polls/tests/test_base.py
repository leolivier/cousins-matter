...
class PollTestBase(MemberTestCase):
  def set_dates(self, pub_days=0, duration=1):
    ...
    def create_question(
        self, poll, question_text, question_type, possible_choices=None
    ):
        """
    ...
    def check_display_info(self, poll, response):
        ...
        self.assertContains(
            response, formats.date_format(time, "SHORT_DATETIME_FORMAT")
        )
...
class PollTestMixin(PollTestBase):
    ...
    def create_poll(
        self, title, description, open_to=Poll.OPEN_TO_ACTIVE, pub_days=-1, duration=2
    ):
        """
...
class EventPlannerTestMixin(PollTestBase):
    ...
    def create_and_check_answers(
        self, event_planner, question, expected_poll_answers=1
    ):
        ...
        nb_dates = (
            1
            if date_question.question_type == Question.SINGLEEVENTPLANNING_QUESTION
            else 2
        )
        ...
        self.assertEqual(PollAnswer.objects.count(), expected_poll_answers)
        poll_answer = PollAnswer.objects.filter(
            poll=event_planner, member=self.current_user()
        ).first()
