...
class PollsVoteView(LoginRequiredMixin, generic.View):
  ...
  def get_question_forms(self, poll):
    ...
    return [{"question": question, "form": self.get_question_form(poll_answer, question)} for question in poll.questions.all()]

  def get_question_form_classes(self, poll):
    return [
      {"question": question, "form": get_answerform_class_for_question_type(question.question_type)}
  ...
  def get(self, request, poll_id):
    ...
    return render(
      request, self.template_name, {"poll": poll, "questions": question_forms, "type": self.poll_model.__name__.lower()}
    )
