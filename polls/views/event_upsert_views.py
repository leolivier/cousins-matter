...
class EventPlannerCreateView(PollCreateView):
    ...
    success_message = _(
        "Event planner created successfully. You can now add other questions if needed."
    )
    ...
    def post(self, request):
        ...
        if form.is_valid():
            ...
            question_text = (
                _("Choose dates") if multichoices_planner else _("Choose one date")
            )
            Question.objects.create(
                question_type=Question.MULTIEVENTPLANNING_QUESTION
                if multichoices_planner
                else Question.SINGLEEVENTPLANNING_QUESTION,
...
class EventPlannerUpdateView(PollUpdateView):
    ...
    def post(self, request, pk):
        ...
        if form.is_valid():
            ...
            managed_closed_list(planner, form)
            multichoices_planner = form.cleaned_data["multichoices_planner"]
            possible_dates = Question.objects.filter(
                poll=planner, question_type__in=Question.EVENT_TYPES
            ).first()
            if possible_dates:
                ...
                if answers:
                    if (
                        ...
                        and possible_dates.question_type
                        == Question.SINGLEEVENTPLANNING_QUESTION
                    ) or (
                        not multichoices_planner
                        and possible_dates.question_type
                        == Question.MULTIEVENTPLANNING_QUESTION
                    ):
                        messages.error(
                            request,
                            _(
                                "This question has already been answered. You can't change its type anymore."
                            ),
                        )
                        ...
                        messages.warning(
                            request,
                            _(
                                "This question has already been answered. Previous answers might be ignored."
                            ),
                        )
                ...
                if multichoices_planner and possible_dates.question_text == _(
                    "Choose one date"
                ):
                    possible_dates.question_text = _("Choose dates")
                elif not multichoices_planner and possible_dates.question_text == _(
                    "Choose dates"
                ):
