...
def create_superuser_from_env():
  ...
  try:
    Member.objects.create_superuser(
      username=username, email=email, password=password, first_name=first_name, last_name=last_name, birthdate=birthdate
    )
  ...
  try:
    ...
    if not su.is_superuser:
      error(2, f"Superuser creation failed: user {username} was created but is not a superuser")
