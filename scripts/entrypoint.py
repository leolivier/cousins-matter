...
def wait_init_and_exec(wait_time=10):
    ...
    logger.error(
        f"Init not finished in {wait_time} seconds, please remove {INIT_IN_PROGRESS_DIR} manually. Exiting..."
    )
    sys.exit(1)
...
def acquire_lock():
    ...
    try:
        INIT_IN_PROGRESS_DIR = (
            settings.BASE_DIR / "config" / ".init_in_progress"
        )  # lock dir
        INIT_IN_PROGRESS_DIR.mkdir()
        logger.info("Initialization lock acquired.")
    except FileExistsError:
        logger.info(
            "Failed to acquire lock: Another container is already initializing the environment."
        )
...
def wait_for_db():
    ...
    for i in range(MAX_RETRIES):
        try:
            ...
            if i < MAX_RETRIES - 1:
                logger.debug(
                    f"Database unavailable ({i + 1}/{MAX_RETRIES}), waiting {RETRY_DELAY} seconds..."
                )
                time.sleep(RETRY_DELAY)
            else:
                logger.error(
                    f"Error: Database connection failed after {MAX_RETRIES * RETRY_DELAY} seconds."
                )
