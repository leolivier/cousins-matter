...
def run(cmd: list[str], check=True, capture_output=False, text=True, quiet=False, cwd: str | None = None):
  ...
  verbose("$", " ".join(cmd))
  return subprocess.run([clean_secret_mark(c) for c in cmd], check=check, capture_output=capture_output, text=text, cwd=cwd)
...
def check_cousins_matter_is_down():
  ...
  try:
    result = run(
      ["docker", "ps", "-a", "--filter", "name=cousins-matter", "--format", "json"], check=True, capture_output=True, text=True
    )
...
please remove the container before running this script""")
  ...
  result = run(
    ["docker", "network", "ls", "--filter", "name=cousins_matter", "-q"], check=True, capture_output=True, text=True
  )
...
from GitHub, and rotates the secret key.
  ...
  p_migrate.add_argument(
    "-d", "--directory", dest="directory", default=os.getcwd(), help="cousins-matter directory (default: current directory)"
  )
  p_migrate.add_argument(
    "-b",
    "--branch",
