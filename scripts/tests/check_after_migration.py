...
def check_members(cursor: sqlite3.Cursor):
    ...
    if len(migrated_members) != len(old_members):
        raise Exception(
            f"Number of members does not match after migration: before {len(old_members)}, "
...
def check_galleries(cursor: sqlite3.Cursor):
    ...
    if len(migrated_galleries) != len(old_galleries):
        raise Exception(
            f"Number of galleries does not match after migration: before {len(old_galleries)}, "
...
def check_photos(cursor: sqlite3.Cursor):
    ...
    if len(migrated_photos) != len(old_photos):
        raise Exception(
            f"Number of photos does not match after migration: before {len(old_photos)}, "
...
def check_forums(cursor: sqlite3.Cursor):
    ...
    if len(migrated_posts) != len(old_posts):
        raise Exception(
            f"Number of posts does not match after migration: before {len(old_posts)}, "
...
def check_classified_ads(cursor: sqlite3.Cursor):
    ...
    for old_classified_ad, migrated_classified_ad in zip(
        old_classified_ads, migrated_classified_ads
    ):
        if (
...
def check_polls(cursor: sqlite3.Cursor):
    ...
    if len(migrated_polls) != len(old_polls):
        raise Exception(
            f"Number of polls does not match after migration: before {len(old_polls)}, "
...
def check_chat(cursor: sqlite3.Cursor):
    ...
    if len(migrated_chatrooms) != len(old_chatrooms):
        raise Exception(
            f"Number of chatrooms does not match after migration: before {len(old_chatrooms)}, "
    ...
    for old_chatmessage, migrated_chatmessage in zip(
        old_chatmessages, migrated_chatmessages
    ):
        if (
...
def check_troves(cursor: sqlite3.Cursor):
    ...
    if len(migrated_troves) != len(old_troves):
        raise Exception(
            f"Number of troves does not match after migration: before {len(old_troves)}, "
