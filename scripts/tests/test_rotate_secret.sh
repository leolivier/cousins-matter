...
python "$script_dir/manage_cousins_matter.py" rotate-secrets || error 1 "rotate-secrets command failed"
...
case "$new_prev_secret" in
    *"$old_secret") : ;;
    *) error 4 "Old secret is not in the list of new previous secrets" ;;
esac
cd || true
rm -rf "$tmp_dir"
